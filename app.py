...
import re
import threading  # 裏側で非同期処理を行うための標準部品
...
# ----------------------------------------------------
# ⚡ 爆速化：バックグラウンド非同期＆スマート差分同期ロジック
# ----------------------------------------------------
def bg_smart_climate_sync():
  """画面を止めずに裏側でこっそり同期し、今日のデータがあれば即終了する"""
  ...
  try:
    ...
    # 既に今日のデータが存在していれば、通信すら一切行わずに0秒で終了
    if latest_date_in_db == today_str:
      return

    # 2. 取得が必要な範囲を設定（基本は6月1日〜今日、既存があれば最新日の翌日〜今日）
...
# バックグラウンドスレッドを起動（ユーザーの画面描画を1ミリ秒もブロックしない）
...
st.sidebar.markdown(
    f"👤 **ログイン中**: `{current_user['username']}`"
    f" （{'👑管理者' if is_admin else '🌱一般スタッフ'}）"
)
...
if menu == "ホーム・本日の状況":
  ...
  if kankyo_logs:
    ...
    if l_wtemp and l_wtemp > 22.0:
      st.error(
          f"⚠️ **【ハウス水温アラート】** 水温が **{l_wtemp}℃** と高めです！"
      )
      st.markdown("---")
  ...
  with right_col:
    ...
    st.checkbox(
        "気象データを自動取り込み・確認する", key="task1", value=True
    )
...
# ----------------------------------------------------
  ...
  with st.form("kankyo_form"):
    ...
    if submitted:
      ...
      update_house_manual_kankyo(str_date, house_temp, water_temp, ec, ph, memo)
      st.success(
          "指定日のハウス内データ（内気温・水温・EC・pH）を安全に更新・保存しました！"
      )
      st.rerun()
    ...
    if submitted:
      if not lines:
        ...
        for l in lines:
          ...
          for b in beds:
            insert_shukaku_and_clear_teichaku(
                str_shukaku_date, full_house, b, weight, quality, memo
            )
            count += 1

        st.success(
            f"🎉 収穫完了！ {house} の {len(lines)}ライン ×"
...
# ----------------------------------------------------
  ...
  st.info(
      "🧠 **AI収量予測精度モデル**：過去の収穫実績より、現在の重量推定制精度は"
  ...
  if not teichaku_records:
    ...
    st.download_button(
        ...
        file_name=(
            f"sanchu_active_lots_{datetime.now().strftime('%Y%m%d')}.csv"
        ),
        mime="text/csv",
    )
    st.markdown("---")
    ...
    h_col1, h_col2, h_col3, h_col4, h_col5, h_col6, h_col7, h_col8 = st.columns(
        [2.2, 1.2, 1.0, 1.1, 1.0, 1.5, 1.2, 0.8]
    )
...
# ----------------------------------------------------
  ...
  if not shukaku_logs:
    ...
    df_s["重量(g)"] = pd.to_numeric(
        df_s["重量(g)"], errors="coerce"
    ).fillna(0.0)
    ...
    st.download_button(
        ...
        file_name=(
            f"sanchu_all_harvest_{datetime.now().strftime('%Y%m%d')}.csv"
        ),
        mime="text/csv",
    )
    st.markdown("---")
    ...
    st.caption(
        "※7日より前の過去データは、最上部の「全過去収穫実績データをCSVダウンロード」ボタンから保存・確認できます。"
    )
    ...
    if df_1week.empty:
      ...
      h1, h2, h3, h4, h5, h6, h7, h8 = st.columns(
          [1.3, 1.2, 2.0, 1.2, 0.9, 1.3, 1.6, 0.7]
      )
      h1.markdown("**収穫日**")
      h2.markdown("**品種**")
      h3.markdown("**栽培場所**")
      ...
      for idx, row in df_1week_sorted.iterrows():
        ...
        d_str = str(row["収穫日"])
        date_fmt = (
            f"{d_str[:4]}/{d_str[4:6]}/{d_str[6:]}" if len(d_str) == 8 else d_str
        )
        ...
        if c8.button(
            "🗑️", key=f"del_shukaku_{row['ID']}", help="この収穫記録を削除"
        ):
  ...
  if not logs:
    ...
    if df_filtered.empty:
      ...
      if df_ht.empty and df_wt.empty:
        st.caption(
            "※ 🏠ハウス内気温 および 💧ハウス水温"
            " は『今日の環境入力』メニューから手動保存した日のみグラフ上にドット描画されます。"
        )
...
# ----------------------------------------------------
  ...
  st.write(
      "現場スタッフや管理者のユーザーアカウント（ID・PW・権限）をここで発行・削除できます。"
  )
      ...
      if submit_u:
        if not new_u or not new_p:
          ...
          if insert_user(new_u, new_p, role_code):
            st.success(
                f"🎉 ユーザー `{new_u}` （権限: {role_code}） を作成しました！"
            )
            st.rerun()
          else:
            st.error(
