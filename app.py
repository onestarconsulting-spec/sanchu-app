...
if menu == "ホーム・本日の状況":
            ...
            try:
                ...
                while current_dt <= today_dt:
                    ...
                    if date_str in kankyo_dict:
                        ...
                        if day["water_temp"] and day["water_temp"] > 22.0: water_stress_days += 1
    ...
    with right_col:
        ...
        st.checkbox("ハウス気温・水温・EC・pHを手動測定して更新する", key="task2")
        ...
        if submitted:
            if not lines:
                ...
                for l in lines:
                    ...
                    for b in beds:
                        insert_teichaku(variety, full_house, b, str_plant_date, int(quantity), str_target_size, memo, created_by=user_disp_name)
                        count += 1
                st.success(f"🎉 担当者: `{user_disp_name}` として {house} の {len(lines)}ライン × {len(beds)}ベッド（計 {count} 件）を一括登録しました！")
...
# ----------------------------------------------------
    ...
    with st.form("kankyo_form"):
        ...
        house_temp = st.number_input("ハウス内気温 (℃) [手動測定]", value=def_htemp, step=0.1)
        water_temp = st.number_input("ハウス水温 (℃) [手動測定]", value=def_water, step=0.1)
        ...
        submitted = st.form_submit_button("この日のハウス内環境データを保存する")
        if submitted:
            str_date = date_val.strftime("%Y%m%d")
            update_house_manual_kankyo(str_date, house_temp, water_temp, ec, ph, memo, created_by=user_disp_name)
            st.success(f"担当者: `{user_disp_name}` として指定日のデータ（内気温・水温・EC・pH）を更新・保存しました！")
            st.rerun()
...
# ----------------------------------------------------
    ...
    st.header("【収穫データ登録】")
    st.write("収穫を行った棟・ライン・ベッドを選択して登録します。登録完了と同時に、**AI収穫予測（栽培管理表）から自動削除**されます。")
        ...
        if submitted:
            if not lines:
                ...
                for l in lines:
                    full_house = f"{house} ({l}ライン)"
                    for b in beds:
                        insert_shukaku_and_clear_teichaku(str_shukaku_date, full_house, b, weight, quality, memo, created_by=user_disp_name)
                        count += 1
                        
                st.success(f"🎉 担当者: `{user_disp_name}` として {house} の {len(lines)}ライン × {len(beds)}ベッド（計 {count} 件）を収穫登録しました！")
...
# ----------------------------------------------------
    ...
    if not teichaku_records:
        ...
        df_export = pd.DataFrame(teichaku_records, columns=["ID", "品種", "ハウス", "ベッド", "定植日", "株数", "予定サイズ", "メモ", "登録日時", "登録者"])
        st.download_button(
        ...
        for record in teichaku_records:
            ...
            created_by_user = record[9] if len(record) > 9 and record[9] else "システム"
            clean_date = plant_date.strip().replace("/", "").replace("-", "")
        ...
        if is_admin: col_widths.append(0.6)
        ...
        if is_admin: h_cols[8].markdown("**操作**")
        st.markdown("---")
        ...
        for lot in lots_data:
            ...
            if is_admin:
                if c_cols[8].button("🗑️", key=f"del_pred_{lot['id']}", help="手動削除"):
...
# ----------------------------------------------------
    ...
    if not shukaku_logs:
        ...
        df_s = pd.DataFrame(shukaku_logs, columns=[
            "ID", "収穫日", "ハウス", "ベッド", "重量(g)", "株数", "品質", "備考", "登録日時", "品種", "登録者"
        ])
        ...
        if df_1week.empty:
            ...
            if is_admin: s_col_widths.append(0.6)
            ...
            if is_admin: sh_cols[8].markdown("**操作**")
            st.markdown("---")
            ...
            for idx, row in df_1week_sorted.iterrows():
                ...
                sc_cols[7].write(str(row['登録日時'])[:16] if row['登録日時'] else "-")
                
                if is_admin:
                    if sc_cols[8].button("🗑️", key=f"del_shukaku_{row['ID']}", help="この収穫記録を削除"):
...
# ----------------------------------------------------
    ...
    if not logs:
        ...
        if df_filtered.empty:
            ...
            if not df_wt.empty:
                fig1.add_trace(go.Scatter(x=df_wt["dt"], y=df_wt["water_temp"], mode='lines+markers', name='💧 ハウス水温 (℃)', line=dict(color='#00bcd4', width=2)))
            ...
            if df_ec_ph.empty:
                st.info("ℹ️ 選択された期間内に手動測定された EC / pH データはありません。「今日の環境入力」から入力してください。")
            else:
                fig4 = make_subplots(specs=[[{"secondary_y": True}]])
                
                fig4.add_trace(go.Scatter(x=df_ec_ph["dt"], y=df_ec_ph["ec"], mode='lines+markers', name='EC (dS/m)', line=dict(color='#e91e63', width=2.5)), secondary_y=False)
                fig4.add_trace(go.Scatter(x=df_ec_ph["dt"], y=df_ec_ph["ph"], mode='lines+markers', name='pH', line=dict(color='#00bcd4', width=2.5, dash='dash')), secondary_y=True)
            ...
            if submit_u:
                if not new_u or not new_p:
                    ...
                    if insert_user(new_u, new_p, role_code, new_disp):
                        st.success(f"🎉 ユーザー `{new_u}` （氏名: {new_disp or new_u}） を作成しました！")
                        st.rerun()
                    else:
